...
func TestPredictTimeSeries(t *testing.T) {
	...
	p := NewPredictor(3, 24)
	predictTs, err := p.PredictTimeSeries(context.TODO(), nil, history)
	assert.NoError(t, err)
	assert.Equal(t, len(predictTs.Samples), 24)

	predictTs2, err := p.PredictTimeSeries(context.TODO(), nil, generateRandTimeSeries())
	assert.NoError(t, err)
	assert.Equal(t, len(predictTs2.Samples), 24)

	p = NewPredictor(3, 1)
	predictTs3, err := p.PredictTimeSeries(context.TODO(), nil, history)
	assert.NoError(t, err)

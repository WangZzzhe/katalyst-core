...
func (p *Predictor) PredictTimeSeries(ctx context.Context, args *common.PredictArgs, history *common.TimeSeries) (*common.TimeSeries, error) {
	...
	for i := range historyBuckets {
		peak := p.predictPeak(historyBuckets[i], args.WorkloadName)
...
func (p *Predictor) predictPeak(samples []common.Sample, name string) float64 {
	m := mean(samples)
	std := stdev(samples, m)

...
func TestSample(t *testing.T) {
	...
	f := &InspectorMetricsFetcher{
		podFetcher: podFetcher,
		client:     client.NewInspectorClient(podFetcher, makeTestMetricClient()),
	...
	data, err := f.GetNodeMetric(consts.MetricMemTotalSystem)
	require.NoError(t, err)
	require.Equal(t, 32614152.0, data.Value)

	data, err = f.GetNodeMetric(consts.MetricMemFreeSystem)
	require.NoError(t, err)
	require.Equal(t, 12891380.0, data.Value)

	data, err = f.GetNodeMetric(consts.MetricMemUsedSystem)
	require.NoError(t, err)
	require.Equal(t, 968376.0, data.Value)

	data, err = f.GetNodeMetric(consts.MetricMemScaleFactorSystem)
	require.NoError(t, err)
	require.Equal(t, 1000.0, data.Value)

	data, err = f.GetNodeMetric(consts.MetricMemPageCacheSystem)
	require.NoError(t, err)
	require.Equal(t, 17783636.0, data.Value)

	data, err = f.GetNodeMetric(consts.MetricMemBufferSystem)
	require.NoError(t, err)
	require.Equal(t, 365640.0, data.Value)

	data, err = f.GetNodeMetric(consts.MetricMemKswapdstealSystem)
	require.NoError(t, err)
	require.Equal(t, 7067178.0, data.Value)

	data, err = f.GetNumaMetric(0, consts.MetricMemTotalNuma)
	require.NoError(t, err)
	require.Equal(t, float64(33396891648<<10), data.Value)

	data, err = f.GetNumaMetric(0, consts.MetricMemFreeNuma)
	require.NoError(t, err)
	require.Equal(t, float64(13186412544<<10), data.Value)

	data, err = f.GetCPUMetric(1, consts.MetricCPUUsageRatio)
	require.NoError(t, err)
	require.Equal(t, 0.04, data.Value)

	data, err = f.GetCPUMetric(6, consts.MetricCPUSchedwait)
	require.NoError(t, err)
	require.Equal(t, 0.0, data.Value)

	data, err = f.GetContainerMetric("2126079c-8e0a-4cfe-9a0b-583199c14027", "testContainer", consts.MetricLoad1MinContainer)
	require.NoError(t, err)
	require.Equal(t, 0.54, data.Value)

	data, err = f.GetContainerMetric("2126079c-8e0a-4cfe-9a0b-583199c14027", "testContainer", consts.MetricLoad5MinContainer)
	require.NoError(t, err)
	require.Equal(t, 0.54, data.Value)

	data, err = f.GetContainerMetric("2126079c-8e0a-4cfe-9a0b-583199c14027", "testContainer", consts.MetricCPUNrRunnableContainer)
	require.NoError(t, err)
	require.Equal(t, 54.0, data.Value)

	data, err = f.GetContainerMetric("2126079c-8e0a-4cfe-9a0b-583199c14027", "testContainer", consts.MetricCPUUsageContainer)
	require.NoError(t, err)
	require.Equal(t, 0.99, data.Value)

	data, err = f.GetContainerMetric("2126079c-8e0a-4cfe-9a0b-583199c14027", "testContainer", consts.MetricMemRssContainer)
	require.NoError(t, err)
	require.Equal(t, 16220160.0, data.Value)

	data, err = f.GetContainerMetric("2126079c-8e0a-4cfe-9a0b-583199c14027", "testContainer", consts.MetricMemCacheContainer)
	require.NoError(t, err)
	require.Equal(t, 0.0, data.Value)

	data, err = f.GetContainerMetric("2126079c-8e0a-4cfe-9a0b-583199c14027", "testContainer", consts.MetricMemShmemContainer)
	require.NoError(t, err)
	require.Equal(t, 0.0, data.Value)

	data, err = f.GetContainerNumaMetric("2126079c-8e0a-4cfe-9a0b-583199c14027", "testContainer", "0", consts.MetricsMemFilePerNumaContainer)
	require.NoError(t, err)
...
func makeTestMetricClient() client.MetricFunc {
	return func(url string, params map[string]string) ([]byte, error) {
		...
		case fmt.Sprintf("%s%s", "http://localhost:9102", types.ContainerCghardwarePath):
			return []byte(`{"data":{"1df178c3a7bddf1269d18b556c1d22a025bdf194c4617d9b411d0ef4b9229ec6":[{"key":"cghardware_cycles","val":0},{"key":"cghardware_instrcutions","val":0}]}}`), nil

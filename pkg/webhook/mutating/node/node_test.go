...
func TestMutateNode(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			expPatch: `[{"op":"replace","path":"/status/allocatable/cpu","value":"88"},{"op":"replace","path":"/status/allocatable/memory","value":"239659175116"}]`,
			allow:    true,
		},
		{
		...
		},
		{
			...
			expPatch: `[{"op":"replace","path":"/status/allocatable/cpu","value":"88"}]`,
			allow:    true,
		},
		{
			...
			expPatch: `[{"op":"replace","path":"/status/allocatable/memory","value":"239659175116"}]`,
			allow:    true,
		},
		{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			assert.Equal(t, string(gotResponse.Patch), c.expPatch)
		})
	}
}

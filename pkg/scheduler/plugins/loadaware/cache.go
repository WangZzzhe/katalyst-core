...
type PodPortraitLister interface {
...
type Cache struct {
	...
	// node predict usage cache is calculated by portrait from portraitLister,
	...
	podPortraitLister PodPortraitLister
...
func (c *Cache) SetPortraitLister(lister PodPortraitLister) {
	if c.podPortraitLister != nil {
...
func (nc *NodeCache) addPod(pod *v1.Pod, time time.Time, portraitLister PodPortraitLister, nodeName string) {
	podUID := string(pod.UID)
	nc.Lock()
	defer nc.Unlock()

	_, ok := nc.PodInfoMap[podUID]
	if ok {
		return
	}
	...
	// update node usage if podPortraitLister is not nil
	if portraitLister != nil {
		podPortrait := portraitLister.GetPodPortrait(pod)
...
func (nc *NodeCache) removePod(pod *v1.Pod, portraitLister PodPortraitLister, nodeName string) int {
	podUID := string(pod.UID)
	nc.Lock()
	defer nc.Unlock()
